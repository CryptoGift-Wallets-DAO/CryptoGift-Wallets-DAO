  ...
  return (
    <div className={`${className}`}>
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-lg font-bold text-white">{t('featured.title')}</h2>
          ...
          return (
            ...
            >
              ...
              <div className="absolute top-3 right-3 flex gap-1">
                {task.is_urgent && (
                  <span className="px-2 py-0.5 text-xs rounded-full bg-red-500/20 text-red-400 border border-red-500/30 animate-pulse">
                    🔥 {t('badges.urgent')}
                  </span>
                )}
              ...
              {/* Title */}
              <h3 className="font-semibold text-white mb-2 line-clamp-2 pr-16">{task.title}</h3>

              {/* Description preview */}
              {task.description && (
                <p className="text-sm text-white/50 line-clamp-2 mb-3">{task.description}</p>
              )}
      ...
      {/* Divider */}
      <div className="mt-6 mb-4 h-px bg-gradient-to-r from-transparent via-white/10 to-transparent" />
    </div>
  )
}
