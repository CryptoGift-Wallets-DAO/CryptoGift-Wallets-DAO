...
import GalleryVideoPlayerEN from '@/components-en/video/GalleryVideoPlayerEN';
...
/**
 * SalesBlockType - Extended for V2 video funnel
 * Legacy types: 'opening' | 'problem' | 'solution' | 'demo' | 'comparison' | 'cases' | 'business' | 'roadmap' | 'close' | 'capture' | 'success'
 * V2 types: 'video' | 'checkpoint' | 'social' (in addition to legacy)
 */
type SalesBlockType =
  // V2 types (NEW - video funnel)
  | 'video'       // Video content block (uses IntroVideoGate)
  | 'checkpoint'  // Simple emotional engagement (yes/no, not quiz)
  | 'social'      // Social verification (Twitter/Discord)
  // Legacy types (preserved for backwards compatibility)
  | 'demo'        // QR code experience
  | 'capture'     // Role selection
  | 'success'     // Celebration screen
  // Legacy content types (NOT used in V2 but preserved)
  | 'opening' | 'problem' | 'solution' | 'comparison' | 'cases' | 'business' | 'roadmap' | 'close';
...
interface SalesBlock {
  ...
  nextBlock?: string;
  // V2 Properties
  videoConfigKey?: string;       // Key from VIDEO_CONFIG for video blocks
  checkpointQuestion?: string;   // Question text for checkpoint blocks
}
...
/**
 * =============================================================================
 * SALES BLOCKS V2 - VIDEO FUNNEL (NEUROMARKETING APPROACH)
 * =============================================================================
...
const SALES_BLOCKS: SalesBlock[] = [
  ...
  // =============================================================================
  {
    ...
    content: {
      ...
      message: '🎁 Complete this video-funnel and receive 200 CGC'
    },
  ...
  // =============================================================================
  {
    ...
    content: {
      ...
      emphasis: 'There is no wrong answer - this is just for you'
    },
  ...
  // =============================================================================
  {
    ...
    content: {
      ...
      emphasis: 'We are early. The infrastructure is ready. The community is forming.'
    },
  ...
  // =============================================================================
  {
    ...
    content: {
      ...
      description: 'Connect with other early adopters',
      emphasis: 'Where the magic happens every day'
    },
  ...
  // =============================================================================
  {
    ...
    content: {
      ...
      message: 'Thank you for completing the Video Funnel',
      stats: {
        duration: '7 minutes',
...
const SalesMasterclassEN: React.FC<SalesMasterclassProps> = ({
  ...
  // State
  // 🔒 V2 UPDATE: video1 ("The Gift") is now shown in the invite page
  // The masterclass now starts directly at video2 ("The Solution")
  // Index mapping: 0=video1(skipped), 1=checkpoint(skipped), 2=video2(START HERE)
  const VIDEO2_START_INDEX = 2; // video2 "The Solution" is at index 2

  const [showIntroVideo, setShowIntroVideo] = useState(() => {
    // V2 NEVER shows the old intro video - video1 is in the invite page now
    console.log('[SalesMasterclassV2EN] 🎬 V2 mode: video1 is in invite page, starting at video2');
    return false;
  });
    ...
    if (savedEducationState?.currentBlockIndex !== undefined) {
      ...
      if (savedIndex >= 0 && savedIndex < SALES_BLOCKS.length) {
        ...
        if (savedIndex < VIDEO2_START_INDEX) {
          console.log('[SalesMasterclassV2EN] 🔒 Saved index was', savedIndex, '- skipping to video2 at index', VIDEO2_START_INDEX);
  ...
  const renderBlockContent = () => {
    ...
    switch (block.type) {
      ...
      // =============================================================================
      // V2 VIDEO FUNNEL CASES - Neuromarketing approach (TOFU → MOFU → BOFU)
      // =============================================================================
      ...
      case 'checkpoint':
        return <CheckpointBlock
          question={block.checkpointQuestion || 'Does this resonate with you?'}
          title={block.title}
      ...
      case 'social':
        return <SocialBlock
          content={block.content || { title: 'Follow Us', description: 'Join the community' }}
          onComplete={handleNextBlock}
          onPrevious={handlePreviousBlock}
          canGoBack={currentBlock > 0}
...
// =============================================================================
// V2 VIDEO FUNNEL COMPONENTS
// =============================================================================

/**
 * VideoBlock V2 - Renders video content using IntroVideoGate
...
const VideoBlock: React.FC<{
  ...
  <div className="py-8">
    ...
    <RotatePhoneHintCompact locale="en" />
    ...
    >
      <p className="text-sm text-gray-500 dark:text-gray-500 flex items-center justify-center gap-2">
        ...
        <span>Complete the video to continue</span>
      </p>
    </motion.div>
  </div>
);
  ...
  return (
    <div className="py-12 text-center max-w-3xl mx-auto">
      ...
      >
        ...
        <h2 className="text-4xl md:text-5xl font-bold mb-4 bg-gradient-to-r from-purple-600 via-pink-600 to-rose-600 dark:from-purple-400 dark:via-pink-400 dark:to-rose-400 bg-clip-text text-transparent">
          {title}
        </h2>
        {description && (
      ...
      <motion.div
        ...
        animate={{ opacity: 1, scale: 1 }}
        transition={{ delay: 0.2 }}
        className="bg-gradient-to-br from-purple-50 via-pink-50 to-rose-50 dark:from-purple-900/20 dark:via-pink-900/20 dark:to-rose-900/20 rounded-2xl p-8 mb-8 border border-purple-200 dark:border-purple-700"
      >
      ...
      >
        <button
          ...
          className={`group px-8 py-4 rounded-xl font-semibold text-lg transition-all duration-300
            ${hasReflected
        ...
        <button
          ...
          className="group px-8 py-4 rounded-xl font-semibold text-lg transition-all duration-300 bg-white dark:bg-gray-800 border-2 border-purple-300 dark:border-purple-600 text-purple-700 dark:text-purple-300 hover:bg-purple-50 dark:hover:bg-purple-900/30"
        >
          <span className="flex items-center gap-2">
      ...
      <motion.p
        ...
        animate={{ opacity: 1 }}
        transition={{ delay: 0.8 }}
        className="mt-8 text-sm text-gray-500 dark:text-gray-500 italic"
      >
        There is no wrong answer - this is just for you
      </motion.p>
    </div>
  );
};
...
const SocialBlock: React.FC<{
  ...
  const handleTwitterVerify = () => {
    ...
    window.open(
      `/api/auth/twitter?returnUrl=${encodeURIComponent(window.location.href)}${walletParam}`,
      'twitter-auth',
  ...
  const handleDiscordVerify = () => {
    ...
    window.open(
      `/api/auth/discord?returnUrl=${encodeURIComponent(window.location.href)}${walletParam}`,
      'discord-auth',
      `width=${width},height=${height},left=${left},top=${top}`
    );
  };

  // Can proceed when at least one platform is verified
  ...
  return (
    <div className="py-12 text-center max-w-3xl mx-auto">
      ...
      >
        ...
        <p className="text-lg text-gray-600 dark:text-gray-400">
          {content?.description || 'Connect with other early adopters'}
        </p>
      </motion.div>
      ...
      >
        ...
        <button
          ...
          disabled={twitterVerified || isTwitterLoading || twitterVerifying}
          className={`w-full max-w-md mx-auto flex items-center justify-center gap-3 px-6 py-4 rounded-xl font-semibold text-lg transition-all duration-300
            ${twitterVerified
        ...
        >
          {twitterVerified ? (
          ...
          ) : isTwitterLoading || twitterVerifying ? (
            <>
            ...
            <>
              ...
              <span>Follow @CryptoGiftDAO</span>
            </>
          )}
        </button>
        ...
        <button
          ...
          disabled={discordVerified || isDiscordLoading || discordVerifying}
          className={`w-full max-w-md mx-auto flex items-center justify-center gap-3 px-6 py-4 rounded-xl font-semibold text-lg transition-all duration-300
            ${discordVerified
              ? 'bg-green-500 text-white cursor-default'
        ...
        >
          ...
          ) : isDiscordLoading || discordVerifying ? (
            <>
              <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />
              <span>Joining...</span>
            </>
      ...
      {canGoBack && onPrevious && (
        <motion.button
          ...
          transition={{ delay: 0.8 }}
          onClick={onPrevious}
          className="mt-4 text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 flex items-center gap-2 mx-auto"
        >
          <ArrowLeft className="w-4 h-4" />
          Back
        </motion.button>
      ...
      <motion.p
        ...
        className="mt-8 text-sm text-gray-500 dark:text-gray-500 italic"
      >
        {content?.emphasis || 'Where the magic happens every day'}
      </motion.p>
    </div>
  );
};
